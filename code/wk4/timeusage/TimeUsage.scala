...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def helper(columnNames: List[String], primary: List[Column], working: List[Column], other: List[Column]) :
    (List[Column], List[Column], List[Column]) = columnNames match  {
      case Nil => (primary, working, other)
      case name :: tail => {
        if (name.startsWith("t01") || name.startsWith("t03") || name.startsWith("t11") || name.startsWith("t1801") || name.startsWith("t1803")) {
          helper(tail, col(name) :: primary, working, other)
        } else if (name.startsWith("t05") || name.startsWith("t1805")) {
          helper(tail, primary, col(name) :: working, other)
        } else if (name.startsWith("t02") || name.startsWith("t04") || name.startsWith("t06") || name.startsWith("t07") || name.startsWith("t08") ||
          name.startsWith("t09") || name.startsWith("t10") || name.startsWith("t12") || name.startsWith("t13") || name.startsWith("t14") ||
          name.startsWith("t15") || name.startsWith("t16") || name.startsWith("t18")) {
          helper(tail, primary, working, col(name) :: other)
        } else {
          helper(tail, primary, working, other)
        }
      }
    }

    helper(columnNames, List[Column](), List[Column](), List[Column]())
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def roundOne(d:Double) = (d * 10).round / 10d
    ...
    summed
      ...
      .map { case ((working, sex, age), primaryNeeds, work, other) => TimeUsageRow(working, sex, age,  roundOne(primaryNeeds), roundOne(work), roundOne(other))
    }.orderBy('working, 'sex, 'age)
